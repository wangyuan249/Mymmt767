...
def extract_features(model, data_loader, print_freq=10, metric=None):
    ...
    with torch.no_grad():
        for source_i, (imgs, fnames, pids, _) in enumerate(data_loader):
            data_time.update(time.time() - end)

            outputs, original_feas, att_scores = extract_cnn_feature(model, imgs)
            #outputs.shape[64,2048]
            #original_feas.shape [64,2048,16,8]
            #att_scores.shape [64,1,16,8]
            index_img = 0
            ...
            if (source_i + 1) % print_freq == 0:
                print('Extract Features: [{}/{}]\t'
                      'Time {:.3f} ({:.3f})\t'
                      'Data {:.3f} ({:.3f})\t'
                      .format(source_i + 1, len(data_loader),
                              batch_time.val, batch_time.avg,
                              data_time.val, data_time.avg))

    return  features, labels                          ## finetune
    # return features, labels                        ## pretrain
