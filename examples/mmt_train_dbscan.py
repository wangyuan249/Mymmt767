...
sys.path.append("/ext/wzy_uda_reID/lzq/MMT_2N_A+B+ring_correct/")
...
def get_mytrain_loader(dataset, height, width, batch_size, workers,
                    num_instances, iters):
    ...
    if not rmgs_flag:
        ...
        print("sampler", len(sampler), next(iter(sampler)))
    else:
        sampler = None
    train_loader = IterLoader(
                DataLoader(Preprocessor(train_set, root=None,
                            ...
                            shuffle= rmgs_flag, pin_memory=True, drop_last=True), length=iters)


    print('train_loader',train_loader.next())
    return train_loader

def get_train_loader(dataset, height, width, batch_size, workers,
                    num_instances, iters, select=False, trainset=None):
    ...
    train_loader = IterLoader(
                # DataLoader(Preprocessor(train_set, root=dataset.images_dir,   select add
                    DataLoader(Preprocessor(train_set, root=None,
                                        transform=train_transformer, mutual=True),
    ...
    for epoch in range(args.epochs):
        ...
        train_loader_target = get_train_loader(dataset_target, args.height, args.width,
                                            args.batch_size, args.workers, args.num_instances,
                                               iters, select= False, trainset=new_dataset,)
        ...
        print("dataset_target.train ", dataset_target.train.__class__)
        print("dataset_target.train len ", len(dataset_target.train))
        train_loader_target.new_epoch()
        # select_dataset, fpath_list, label_pathonly_list = trainer.select_byentropy(
        #     epoch = epoch,data_loader_target=train_loader_target, len_dataset=len(dataset_target.train),
        #     batch_size=args.batch_size)
        # # print(fpath_list[:100])
        # whole_path = []
        # for w_item in dataset_target.train:
        #     whole_path.append(w_item[0])
        # # whole_path = list(dict_f.keys())
        #
        # # pdb.set_trace()
        # m = n =0
        # for myitem in label_pathonly_list:
        #     print(myitem)
        #     try:
        #         whole_path.remove(str(myitem))
        #         m += 1
        #         print("m", m)
        #     except:
        #         print("no path in whole")
        #         n += 1
        #         print("n", n)
        #
        #
        # unlabel_pathonly_list = whole_path
        #
        # unlabel_fpath_list = []
        # for item in unlabel_pathonly_list:
        #     mytuple = (item, 1, 1)
        #     unlabel_fpath_list.append(mytuple)
        #
        # # unlabel_fpath_list = dataset_target.train.remove(fpath_list)
        # print("unlabel_fpath_list ", unlabel_fpath_list.__class__)
        # print("unlabel_fpath_list len ", len(unlabel_fpath_list))
        # # pdb.set_trace()
        #
        # # unlabel_train_loader_target = get_mytrain_loader(dataset=unlabel_fpath_list, height=args.height, width=args.width,
        # #     batch_size=args.batch_size, workers=args.workers, num_instances = args.num_instances, iters=iters,select=True, trainset=None)
        # mytrain_loader_target = get_mytrain_loader(unlabel_fpath_list, args.height, args.width,
        #                                        args.batch_size, args.workers, args.num_instances, iters)
        # mytrain_loader_target.new_epoch()
        # pdb.set_trace()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--init-1', type=str,
                        # default= "/ext/wzy_uda_reID/mmt_0401/MMT_2N_A+B+ring_correct/examples/logs/market/resnet50-pretrain-7/model_best.pth.tar",
                        # default= "/ext/wzy_uda_reID/mmt_0401/MMT_2N_A+B+ring_correct/examples/logs/duke/resnet50-pretrain-4/model_best.pth.tar",
                        # metavar='PATH')
                        default="../examples/logs/market/resnet50-pretrain-delg-1/model_best.pth.tar", metavar='PATH')
                        # default='../model/model_best.pth.tar', metavar='PATH')
    parser.add_argument('--init-2', type=str,
                        # default="/ext/wzy_uda_reID/mmt_0401/MMT_2N_A+B+ring_correct/examples/logs/market/resnet50-pretrain-6/model_best.pth.tar",
                        # default="/ext/wzy_uda_reID/mmt_0401/MMT_2N_A+B+ring_correct/examples/logs/duke/resnet50-pretrain-5/model_best.pth.tar",
                        # metavar='PATH')
                        default="../examples/logs/market/resnet50-pretrain-delg-2/model_best.pth.tar", metavar='PATH')
                        # default='../model/model_best.pth.tar', metavar='PATH')
    ...
    parser.add_argument('--logs-dir', type=str, metavar='PATH',
                        default=osp.join(working_dir, 'logs/duke/dukeTOmarket/MMT-best-delg420'))
    main()
