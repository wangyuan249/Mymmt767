...
# from examples.DELF.delf import Delf_V1
sys.path.append("../")
...
def create_model(args):
    ## Create model
    # from examples.DELF.delf import Delf_V1
    # model_1 = Delf_V1(ncls=args.num_clusters, load_from="model_path", arch=args.arch,
    #     stage='finetune', target_layer='layer3', use_random_gamma_rescale=False)
    #
    # model_2 = Delf_V1(ncls=args.num_clusters, load_from="model_path", arch=args.arch,
    #                   stage='finetune', target_layer='layer3', use_random_gamma_rescale=False)
    #
    # model_1_ema = Delf_V1(ncls=args.num_clusters, load_from="model_path", arch=args.arch,
    #                   stage='finetune', target_layer='layer3', use_random_gamma_rescale=False)
    #
    # model_2_ema = Delf_V1(ncls=args.num_clusters, load_from="model_path", arch=args.arch,
    #                       stage='finetune', target_layer='layer3', use_random_gamma_rescale=False)
    ...
    # model_1_ema.module.module_dict['logits'][0].weight.data.copy_(model_1.module.module_dict['logits'][0].weight.data)
    ...
    # model_2_ema.module.module_dict['logits'][0].weight.data.copy_(model_2.module.module_dict['logits'][0].weight.data)
...
def main_worker(args):
    ...
    pca = PCA(n_components=35, whiten=True)#origin 40 or 100

    for epoch in range(args.epochs):
        dict_f, _, featureSelect, = extract_features(model_1_ema, cluster_loader, print_freq=20)
        #####  cf_1 = torch.stack(list(dict_f.values())).numpy()
        #####  print("cf_1.shape", cf_1.shape)
        cf_select1 = torch.stack(list(featureSelect.values())).numpy()  # cf_select1.shape (12936, 131072, 1)
        # print("cf_select1.shape", cf_select1.shape)
        cf_select1 = cf_select1.reshape([np.size(cf_select1,0), -1, 2048])
        print("after reshape cf_select1.shape", cf_select1.shape)
        # temp0_48 = np.zeros((1,48), dtype=float)
        cf_embeds1 = np.empty(0)
        for i in range(0, np.size(cf_select1,0)):
            subPcaTensor = cf_select1[i,:,:]
            newX = pca.fit_transform(subPcaTensor)  ## return shape newX 100*20
            newX = newX.reshape(1,-1)
            # newX = np.concatenate([newX,temp0_48],axis = 1)#选出的2000+空的48
            # print(newX.shape)
            if i == 0:
                cf_embeds1 = newX[:,:2048]
            else:
                cf_embeds1 = np.concatenate((cf_embeds1,newX[:,:2048]),axis=0)

            # print(cf_embeds.shape)
            # print(newX.shape)
            # print(pca.explained_variance_ratio_)
            # mysum = 0.0
            # for item in pca.explained_variance_ratio_[:20] :
            #     print("item: ",item)
            #     mysum += item
            #     print("mysum: ", mysum)
            # print("mysum: ",mysum)
        ...
        dict_f, _, featureSelect = extract_features(model_2_ema, cluster_loader, print_freq=20)
        #####  cf_2 = torch.stack(list(dict_f.values())).numpy()
        cf_select2 = torch.stack(list(featureSelect.values())).numpy()
        cf_select2 = cf_select2.reshape([np.size(cf_select2, 0), -1, 2048])
        print("after reshape cf_select1.shape", cf_select2.shape)

        cf_embeds2 = np.empty(0)
        for i in range(0, np.size(cf_select2, 0)):
            subPcaTensor = cf_select2[i, :, :]
            newX = pca.fit_transform(subPcaTensor)  ## return shape newX 100*40
            newX = newX.reshape(1, -1)
            # newX = np.concatenate([newX, temp0_48], axis=1)
            if i == 0:
                cf_embeds2 = newX[:,:2048]
            else:
                cf_embeds2 = np.concatenate((cf_embeds2, newX[:,:2048]), axis=0)

        #####  cf = (cf_1 + cf_2) / 2

        cf_embeds = (cf_embeds1 +cf_embeds2) / 2
        ...
        km = KMeans(n_clusters=args.num_clusters, random_state=args.seed, n_jobs=2).fit(cf_embeds)

        temp = normalize(km.cluster_centers_, axis=1)
        temp = torch.from_numpy(temp).float().cuda()
        model_1.module.classifier.weight.data.copy_(temp)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num-clusters', type=int, default=600)
    ...
    parser.add_argument('--init-1', type=str,
                        default='/home/aiclub/lzq/mmt37/logs/resnet50-pretrain-4/model_best.pth.tar', metavar='PATH')
    parser.add_argument('--init-2', type=str,
                        default='/home/aiclub/lzq/mmt37/logs/resnet50-pretrain-5/model_best.pth.tar', metavar='PATH')
    ...
    parser.add_argument('--logs-dir', type=str, metavar='PATH',
                        default=osp.join(working_dir, 'logs/duke/dukeTOmarket/resnet50-MMT-Add-600-2'))
    main()
